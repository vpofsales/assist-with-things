 // supabase/functions/gemini-proxy/index.ts
    ...
    serve(async (req: Request) => {
        ...
        try {
            console.log('--- Function Invoked ---'); // Added log
            ...
            if (!geminiApiKey) {
                console.error('GEMINI_API_KEY not set.'); // Added log
                return new Response(
            ...
            if (!oxylabsApiUsername || !oxylabsApiPassword) {
                console.error('OXYLABS_API_USERNAME or OXYLABS_API_PASSWORD not set.'); // Added log
                return new Response(
            ...
            console.log('Received promptText:', promptText); // Added log

            if (!promptText) {
                console.error('promptText is missing.'); // Added log
                return new Response(
            ...
            try {
                ...
                console.error('Error calling Gemini for triage (after fetch):', geminiError.message);
                ...
                console.error('Error calling Gemini for triage:', geminiError.message); // Added log
                extractedSearchQuery = promptText; // Fallback
            }

            console.log('Extracted search query for Oxylabs:', extractedSearchQuery); // Existing log
            ...
            const oxylabsRequestBody = {
                source: 'amazon_search', // Keep this for now, we'll confirm its output
            ...
            console.log('Sending request to Oxylabs with body:', JSON.stringify(oxylabsRequestBody, null, 2)); // Added log
            ...
            console.log('Received response status from Oxylabs:', oxylabsResponse.status); // Added log

            if (!oxylabsResponse.ok) {
                const errorData = await oxylabsResponse.json().catch(() => ({ message: 'Could not parse Oxylabs error response.' })); // Improved error parsing
                console.error('Oxylabs API returned non-OK status:', oxylabsResponse.status, errorData); // Added log
                return new Response(
            ...
            console.log('Oxylabs Raw Response:', JSON.stringify(oxylabsData, null, 2)); // THIS IS THE CRUCIAL LOG
            ...
            if (oxylabsData.content && oxylabsData.content.results && oxylabsData.content.results.length > 0) {
                ...
                console.warn('Oxylabs returned no products or unexpected structure. Defaulting to empty array.'); // Added log
            }
            ...
            console.log('Successfully prepared products for frontend.'); // Added log
            ...
            console.error('Edge Function top-level catch error:', error.message, error.stack); // More detailed error log
            return new Response(
